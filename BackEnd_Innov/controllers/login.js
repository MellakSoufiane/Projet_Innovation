...
async function login(req, res) {
  ...
  if (!userWithEmail) return res.status(401).send({ message: 'Wrong email or password' })

  const isCorrectPassword = await comparePassword(password, userWithEmail.password)

  if (!isCorrectPassword) return res.status(401).send({ message: 'Wrong email or password' })
