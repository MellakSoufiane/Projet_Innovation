...
async function signup(req, res) {
  ...
  //Dummy validation
  if (!email || !password || !name) return res.status(400).send({ message: 'Email,name and password are required' })

  const userWithEmail = await getUserByEmail(email)

  if (userWithEmail) return res.status(409).send({ message: 'User with this Email already exists' })
